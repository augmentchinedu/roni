  ...
  for (const candidate of candidates) {
    try {
      ...
      if (candidate.includes("/") || candidate.includes("\\")) {
        ...
        const cmd =
          platform === "win32" ? `where ${candidate}` : `which ${candidate}`;
...
async function downloadChromium() {
  console.log(
    "[boot] Chromium not found. Auto-downloading via @puppeteer/browsers..."
  );
  try {
    ...
    execSync("npm install --no-save @puppeteer/browsers", {
      cwd: ROOT,
      stdio: "inherit",
    });
    ...
    console.error(
      "[boot] Please install Chrome manually: https://www.google.com/chrome/"
    );
    process.exit(1);
  }
}
...
async function spawnChromium(config, port) {
  ...
  console.log(
    `[boot] Launching Chromium: ${bin.split(/[/\\]/).pop()} ${args[0]} ...`
  );
  ...
  child.stderr.on("data", (d) => {
    ...
    if (
      msg.includes("Failed to connect to the bus") ||
